...
query_status_t
send_ut2004master_request_packet(struct qserver *server)
{
	int ret;
...
static void
ut2004_server_done(struct qserver *server)
{
...
static void
ut2004_parse_record(struct qserver *server, char *pkt)
{
	char *dest;
...
static char *
put_bytes(char *buf, const char *bytes, size_t len, size_t *left)
{
	if (!buf || (len > *left)) {
		return (NULL);
	}
...
static char *
put_string(char *buf, const char *string, size_t *left)
{
 ...
 */
static int
ut2004_send_query(struct qserver *server)
{
	char buf[4096] = { 0 };
...
query_status_t
deal_with_ut2004master_packet(struct qserver *server, char *rawpkt, int pktlen)
{
...
static void
bin2hex(const char *in, size_t len, char *out)
{
	char *o = out + len * 2;
