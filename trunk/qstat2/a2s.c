...
#define A2S_RULESRESPONSE_HL 0x45
...
void deal_with_a2s_packet(struct qserver *server, char *rawpkt, int pktlen)
{
    ...
    if( 0 == memcmp(pkt, "\xFE\xFF\xFF\xFF", 4) )
	{
		...
		default:
			malformed_packet( server, "unknown packet index" );
			goto out_error;
		}
		pkt++;
