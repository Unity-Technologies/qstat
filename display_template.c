...
static int is_true(struct qserver *server, struct player *player, struct rule *rule, char *expr);
	...
	switch(var) {
		...
		case V_PING:
			if (server->server_name != TIMEOUT && server->server_name != DOWN &&
				...
				fprintf(OF, "%d", server->n_requests ? server->ping_total/server->n_requests : 999);
			}
			break;
		case V_PLAYERS:
			if (full_data)
		...
		case V_GAMETYPE: {
			...
			break;
		case V_RETRIES:
			if (server->server_name != TIMEOUT && server->server_name != DOWN && server->server_name != HOSTNOTFOUND)
				fprintf(OF, "%d", server->n_retries);
			break;
		case V_IPADDR: {
				unsigned int ipaddr= ntohl(server->ipaddr);
				fprintf(OF, "%u.%u.%u.%u", (ipaddr>>24)&0xff, (ipaddr>>16)&0xff, (ipaddr>>8)&0xff, ipaddr&0xff);
			}
			break;
		case V_PORT:
...
display_generic_var(int var)
{
	switch(var) {
		...
		case V_TOTALSERVERS:
			...
			break;
		case V_TOTALUP:
			fprintf(OF, "%d", num_servers_total - num_servers_timed_out - num_servers_down);
			break;
		case V_TOTALNOTUP:
...
parse_var(char *varname, int *varlen)
{
	...
	*varlen= v-varname;
	...
	for (i= 0; i < sizeof(variable_defs)/sizeof(struct vardef); i++)
		if (strncasecmp(varname, variable_defs[i].var, *varlen) == 0 && *varlen == strlen(variable_defs[i].var)) {
			if (colon != NULL && ((variable_defs[i].options & OPTIONS_OK) || quote)) {
				for (v++; *v; v++) {
					if ((!quote && !isalnum((unsigned char)*v) && *v != '*' && *v != '_' && *v != '.' &&
						(*v == ' ' && !rule_name_spaces)) || (quote==1 && *v == ')')) {
						break;
					}
					if (*v == '(')
						quote++;
		...
		char *expr)
{
	...
	for (i= 0; i < sizeof(variable_defs)/sizeof(struct vardef); i++)
		if (strncasecmp(expr, variable_defs[i].var, len) == 0 && len == strlen(variable_defs[i].var)) {
			...
			switch (variable_defs[i].varcode) {
				...
				case V_PLAYERS:
					return server->num_players > 0;
				case V_ISEMPTY:
					return server->num_players == 0;
				case V_ISFULL:
					return server->max_players ? server->num_players >= server->max_players : 0;
				case V_ISTEAM:
					return player ? player->number == TRIBES_TEAM : 0;
				case V_ISBOT:
					return player ? player->type_flag == PLAYER_TYPE_BOT : 0;
				case V_ISALIAS:
					return player ? player->type_flag == PLAYER_TYPE_ALIAS : 0;
				case V_TRIBETAG:
					return player ? player->tribe_tag != NULL : 0;
				case V_RULE:
					 {
						 struct rule *rule;
						 if (arg == NULL)
